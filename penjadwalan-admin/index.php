<?php
	...
	// Menjalankan query untuk mengambil data jadwal kuliah, admin, mata kuliah, dan kelas dari database
	$sql = 	"SELECT nama_lengkap AS `Nama Admin`, WA AS `No Telp`, email AS `Email`, id AS `id`
			FROM user
			WHERE role = 'admin';";
       ...
       $(document).ready(function(){
		  ...
		  // Edit button click event
			$(document).on("click", ".edit", function() {
				...
				var id = $(this).data("id");
				var name = $("#row_" + id).find("td:eq(1)").text();
				var phone = $("#row_" + id).find("td:eq(2)").text();
				var email = $("#row_" + id).find("td:eq(3)").text();
		...
		// Save button click event
			$("#editEmployeeModal").on("click", ".btn-success", function() {
				...
				$("#row_" + id).find("td:eq(3)").text(email);
