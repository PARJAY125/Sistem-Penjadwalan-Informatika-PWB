<?php
	...
	$sql = 	"SELECT nama_kode_kelas AS `Nama Kelas`, jumlah_mahasiswa AS `jumlah`, id AS `id`
			FROM kelas";
...
<div class="wrapper">
	 ...
	 <div id="sidebar">
		...
		<ul class="list-unstyled component m-0">
		  ...
		  <li class="">
		  	<a href="datamahasiswa.php" class="dashboard"><i class="material-icons">library_books</i>Data Mahasiswa </a>
		  </li>
		  ...
		  <div class="top-navbar">
		     <div class="xd-topbar">
				 ...
				 <div class="xp-breadcrumbbar text-center">
				    <h4 class="page-title mt-2">Data Kelas</h4>
				 </div>
			 </div>
		  </div>
		  <!------top-navbar-end-----------> 
								...
								<tr>
									<?php
										while ($row = mysqli_fetch_assoc($result)) {
											...
											echo '</a>';
											echo '<a href="#deleteEmployeeModal" class="delete" data-toggle="modal">';
       ...
       $(document).ready(function(){
		  ...
		  // Edit button click event
			$(document).on("click", ".edit", function() {
				...
				var id = $(this).data("id");
				var namaKelas = $("#row_" + id).find("td:eq(1)").text();
				var jumlah = $("#row_" + id).find("td:eq(2)").text();
				$("#editEmployeeModal").find("input:eq(0)").val(namaKelas);
				$("#editEmployeeModal").find("input:eq(1)").val(jumlah);
		...
		// Save button click event
			$("#editEmployeeModal").on("click", ".btn-success", function() {
				var id = $(this).data("id");
				var namaKelas = $("#editEmployeeModal").find("input:eq(0)").val();
				var jumlah = $("#editEmployeeModal").find("input:eq(1)").val();

				// Update the table row with the edited data
				$("#row_" + id).find("td:eq(1)").text(namaKelas);
				$("#row_" + id).find("td:eq(2)").text(jumlah);

				// Make an AJAX request to update the data in the database
				$.ajax({
					url: "update_kelas.php", // Replace with the URL of your server-side script to update the data
					type: "POST",
					data: { namaKelas: namaKelas, jumlah: jumlah, id: id },
			...
			$(document).on("click", ".delete", function() {
				...
				$.ajax({
					url: "delete_kelas.php", // Replace with the URL of your server-side script to delete the data
					type: "POST",
					data: { id: id },
					success: function(response) {
						console.log(response); // You can handle the response from the server here
